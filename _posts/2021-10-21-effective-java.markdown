...
Рассмотрим следующую реализацию класса `Stack` и метода `pop`:
...
Тут скрывается утечка памяти: элементы, которые были извлечены из стека,
всё ещё хранятся в ячейках под индексами, большими `size`.
Стек хранит устаревшие ссылки, поэтому они не могут быть уничтожены
сборщиком мусора.
Устаревшие ссылки - это ссылки, котрые никогда не будут разрешены.
Такие коварные ошибки в языках со сборщиком мусора приводят к тому,
что не уничтожаются не только устаревшие объекты, но и другие объекты,
на которые ссылаются устаревшие объекты.

В крайнем случае это приводит к сильному замедлению программы из-за постоянных
сборок мусора, перемещению страниц ОЗУ на диск и обратно или даже отказу всей
системы из-за `OutOfMemoryError`.

Решиене - "занулить" ссылки, как только они устаревают.
...
Дополнительное преимущество зануления устарешвих ссылок - если обратиться к
такой ссылке по ошибке, программа сразу закрешится от `NullPointerException`
вместо того, чтобы продолжать работать неправильно.

Не спешите обнулять каждую ссылку, которая перестаёт использоваться. В этом
нет необходимости - это только засоряет исходный код. __Обнуление ссылок
должно быть исключением, а не нормой__. Лучший способ удалить устаревшую
ссылку - дать переменной, которая её содержит, быть уничтоженной вместе с её
областью видимости. Это происходит автоматически, если вы определяете
переменные в как можно более меньшей области видимости.

Когда нужно обнулять ссылки и какой аспект класса `Stack` делает его
подверженным утечкам памяти? __Он сам управляет своей памятью__.

В общем, когда класс управляет своей памятью самостоятельно,
вы должны следить за утечками памяти.
Когда какой-то объект становится не нужен, нужно занулить все ссылки на него.

Другой частый источник утечек памяти - это кеш. Положив объект в кеш, легко
забыть о нём и оставить его там, даже если он становится не нужен.
Есть несолько решений для этой проблемы. Если запись в кеше релевантна, только
когда где-то снаружи сущствует ссылка на объект, то используйте `WeakHashMap`.
В таком случае ссылки автоматически удаляться, как только устареют.

Чаще польза от хранения записи в кеше определена менее чётко, запись становится
менее ценной со временем. В таком случае кеш должен очищаться время от времени
от записей, которые перестали использоваться. Тут подойёт `LinkedHashMap` и его
метод `removeEldestEntry`. Для более сложных правил кеша придётся использовать
`java.lang.ref` напрямую.

Утечки памяти сложно обнаружить, они долго остаются в системе. С этой проблемой
поможет профилировщик кучи (heap profiler). Такие проблемы лучше решать
превентивно.

### 8. Избегайте файналайзеров и клинеров (finalizers and cleaners)
