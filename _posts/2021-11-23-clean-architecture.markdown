...
> Тут нужно что-то вроде следующего свойства подстановки: Если для каждого
  объекта o1 типа S существует объект o2 типа T, такой что для всех программ P,
  определённых в терминах T, поведение P не изменится, когда o2 заменит o1,
  то S - это подтип T.

(В общем надо проектировать компоненты такими, чтобы они были взаимозаменяемы,
если нужно их будет заменять друг на друга).
